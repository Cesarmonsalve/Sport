...
export const TeamLogoLayer = memo(function TeamLogoLayer({
  ...
  const layerId =
    id === legacyId || id === `team-logo-${side}`
      ? id
      : `team-logo-${side === "home" ? "home" : "away"}`;
  ...
  const override = side === "home" ? game.homeLogoOverride : game.awayLogoOverride;
  const custom =
    elements[layerId]?.imageUrl ??
    elements[legacyId]?.imageUrl ??
    binding?.manualImageUrl;
  const src = useEspn ? override ?? espnUrl ?? custom : custom ?? override ?? espnUrl;
